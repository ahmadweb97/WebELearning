...
namespace WebELearning.Areas.Admin.Controllers
{
    [Area("Admin")]
    ...
    public class ContentController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("Id,Title,HTMLContent,VideoLink,CategItemId,CategoryId")] Content content)
        {
            if (ModelState.IsValid)
            {
                content.CategoryItem = await _context.CategoryItem.FindAsync(content.CategItemId);
        ...
        {
            if (categoryItemId == 0 || _context.Content == null)
            {
                return NotFound();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("Id,Title,HTMLContent,VideoLink,CategItemId,CategoryId")] Content content)
        {
            if (id != content.Id)
            {
                return NotFound();
