...
namespace WebELearning.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            return await (from categItem in _context.CategoryItem
                          join category in _context.Category
                          on categItem.CategoryId equals category.Id
                          join content in _context.Content
                          on categItem.Id equals content.CategoryItem.Id
                          join userCat in _context.UserCategory
                          on category.Id equals userCat.CategoryId
                          join mediaType in _context.MediaType
                          on categItem.MediaTypeId equals mediaType.Id
                          where userCat.UserId == userId
                          select new CategoryItemDetailsModel
                          {
                              CategoryId = category.Id,
                              CategoryTitle = category.Title,
                              CategoryItemId = categItem.Id,
                              CategoryItemTitle = categItem.Title,
                              CategoryItemDescription = categItem.Description,
